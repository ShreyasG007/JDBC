...
public class HighestPaidEmployee {
	public void work() throws ClassNotFoundException, SQLException {
		...
		Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/capgemini", "root",
				"Shreyas21");
		String query = "select name,salary from employee where salary=(select max(salary) from employee)";
		Statement statement = connection.createStatement();
		ResultSet rs = statement.executeQuery(query);

		while (rs.next()) {
			String name = rs.getString(1);
			String maxSalary = rs.getString(2);
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException {
		HighestPaidEmployee updateSalary = new HighestPaidEmployee();
		updateSalary.work();
	}
}
